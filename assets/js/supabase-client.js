(() => {
  ...
  async function fetchAllRows() {
    ...
    const rows = [];
    while (true) {
      const { data, error } = await client.from('photos').select('*').order('uploaded_at', { ascending: false }).range(from, from + pageSize - 1);
  ...
  async function fetchPhotos(options = {}) {
    ...
    photos.sort((a, b) => (options.sort === 'oldest' ? 1 : -1) * (photoTime(a) - photoTime(b)) || a.sortOrder - b.sortOrder);
    return photos;
  }
  ...
  window.PhotoArchive = {
    config, configured, client, bucket: BUCKET, escapeHtml, numberOrNull,
